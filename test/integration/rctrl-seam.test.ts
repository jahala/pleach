...
describe.skipIf(SKIP)(`rctrl-seam-${RUN_ID} — happy path`, () => {
  test('spawnWorker + send + wait returns stop with non-empty finalMessage', async () => {
    ...
    SPAWNED.push((worker as unknown as { _name: string })._name);
...
describe.skipIf(SKIP)(`rctrl-seam-${RUN_ID} — two-turn conversation`, () => {
...
describe.skipIf(SKIP)(`rctrl-seam-${RUN_ID} — wait timeout`, () => {
...
describe.skipIf(SKIP)(`rctrl-seam-${RUN_ID} — dead worker`, () => {
  test('worker killed externally returns reason dead', async () => {
    const seam = makeSeam();
    const worker = await seam.spawnWorker({ cwd: '/tmp' });
    const name = (worker as unknown as { _name: string })._name;
    SPAWNED.push(name);
...
describe.skipIf(SKIP)(`rctrl-seam-${RUN_ID} — spawn failure`, () => {
