...
const checks: Check[] = [
  ...
  },
  {
    ...
    id: 'ga.friction',
    claim:
      'A worktree friction journal (`.plotplot/friction/*.jsonl`) is kept as `<node>.friction.jsonl` (months concatenated in name order) with its own `gate-artifact` event, and neither `.plotplot/friction/` nor `.loop-scratch/` is ever staged as delivery',
    evidence: 'test/loop/gate-artifact-friction.test.ts',
    needs: ['ga.settle-write'],
    how: `Two halves. (a) Reading the worktree is seam I/O: IsolateSeam gains \`readFriction(cwd): Promise<string | null>\` (src/loop/deps.ts + src/seams/isolate.ts; the harness implements it from a per-node fixture) that returns the concatenation, in filename order, of every \`.plotplot/friction/*.jsonl\` directly in that directory (not state/, not hotspots.json), or null when the directory or files are absent. At settle (both paths), when it returns text: writeArtifact(node, 'friction', text), journal {event:'gate-artifact', node, gate:'friction', path, sha256: sha256Hex(text)}, receipt \`artifacts.friction\`. (b) Never delivery: a pure predicate in core (e.g. src/core/delivery.ts \`isDeliveryPath(path)\`) that refuses paths under \`.plotplot/friction/\` and \`.loop-scratch/\`, applied where run-node builds the staged set (dedup([...filesTouched, ...changed])) — so neither the worker manifest nor the changedFiles fallback can stage them, whatever the repo's .gitignore says. Prove both on the harness, and the predicate in a unit test inside the same evidence file or beside it.`,
  },
  {
    n: 6,
    id: 'ga.e2e',
