// Hand-split plan for the test-phase-commit loop (D13). tend2 emit-plan emits ONE node per loop
// (jahala/tend#158) and no phased work (dogfood log #12), so this script takes the emitted node's
// prompt scaffolding and writes one PHASED node per check: RED writes the failing test at the
// check's evidence path, IMPL builds, GREEN proves `bun run check`. Every node's smoke is
...
//
//   bun docs/dogfood/test-phase-commit/make-plan.ts > docs/dogfood/test-phase-commit/plan.json

const LOOP = 'docs/tend2/test-phase-commit.tend2.html';
const PAYLOAD = process.env.PAYLOAD ?? '5ae23d02010d';
...
function verify(check?: number): string {
  ...
  return `${VERIFY} verify ${LOOP} --repo-root .${scope} --force --runner 'bun test {evidence}' --audit-egress --expect-payload ${PAYLOAD}`;
}

const ENV = `## Your environment
- You are at the repository root of an isolated git worktree of pleach. Work here directly.
- Read ENGINEERING.md FIRST and conform to it (it is binding: test-first, no stubs/mocks/TODOs, S.U.P.E.R. layering, typed errors, no bare \`new Error\`, never \`rm\`). Then read docs/ledger.md item D13 and the loop file ${LOOP} including its narrative and Tried — it is your full context and it is READ-ONLY for you (check states belong to the verifier; a Tried line moves the payload pin).
...
function prompt(c: Check, phase: 'red' | 'impl' | 'green'): string {
  const head = `You are a worker in a build loop. Make the check below pass, honestly. Nobody reviews your prose — only the verifier's verdict counts.\n\n## The loop\nThe red phase is its own commit (ledger D13; loop ${LOOP}). When a node's work carries a test phase ({test, phases}), pleach seals the red phase as its own commit the moment the RED gate passes — before the impl prompt is sent — so the failing-test state exists in node/<id> history: base → red → verified.\n\n## Your check (c${c.n})\n- (code) ${c.claim} · evidence: ${c.evidence}\n\n## How this node fits\n${c.how}\n\n${ENV}`;
  if (phase === 'red') {
...
const plan = {
  goal: 'The red phase is its own commit (D13) — docs/tend2/test-phase-commit.tend2.html',
  source: LOOP,
  maxConcurrency: 2,
  nodes: checks.map((c) => ({
    id: c.id,
    ...
    accept: {
      ...
      audit: {
        command: verify(c.id === 'tpc.journal-doc' ? undefined : c.n),
        provider: AUDITOR,
        selfIntegrity: true,
      },
    },
    policy: { maxAttempts: 2, timeoutMs: c.timeoutMs ?? 1_800_000 },
    closes: [`test-phase-commit:c${c.n}`],
  })),
};
