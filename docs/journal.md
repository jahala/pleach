...
| `gate-artifact` | `node`, `gate` (`smoke`, `friction`), `path`, `sha256` | something the worktree held was kept beside the receipt before it was disposed (D14). `path` is the close's own file — `<git-dir>/pleach/receipts/<node>.<sha256 prefix><ext>`, the name its receipt carries, with the same bytes under the un-prefixed name for whatever closed last (D17). `smoke`: the gate's stdout when it parses as a SARIF 2.1.0 log (`.sarif`), and `sha256` is the sealed `gates[].artifactSha` — one hash, cited by the receipt and answered by the file. `friction`: the tree's own friction journal (`.plotplot/friction/<yyyy-mm>.jsonl`, months in name order), at `.friction.jsonl`; no gate produced it, so `sha256` is over the bytes kept |
